...
export function useSupabaseSync() {
  ...
  useEffect(() => {
    ...
    const unsub = useStore.subscribe((next) => {
      ...
      syncItems(
        prev.workStatements, next.workStatements,
        (w) => supabase.from("technician_work_statements").upsert(workStatementToRow(w, companyId), { onConflict: "id" }),
        (w) => supabase.from("technician_work_statements").delete().eq("id", w.id),
      );
      ...
      syncItems(
        prev.agentTaxIds, next.agentTaxIds,
        (t) => supabase.from("agent_tax_ids").upsert(agentTaxIdToRow(t, companyId), { onConflict: "agent_id" }),
        (t) => supabase.from("agent_tax_ids").delete().eq("agent_id", t.id),
      );
